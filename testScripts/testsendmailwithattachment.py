...
class sendlogin(unittest.TestCase):
    def testsendmailwithattachment(self):
        ...
        test = waitvisibilityofelementlocated('xpath','//input[@name = "password"]')
        assert_attribute(test,'autocomplete')
        assert_get_html(test,'autocomplete')
        attribute()
        print(u'输入的密码为：', pwd)
        print(u'登陆中请稍后.....')
        #调用action的定位并点击方法
